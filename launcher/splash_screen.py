...
class SplashScreen(QtWidgets.QSplashScreen):
    ...
    def __init__(self, filename=None):
        ...
        if geometry != None:
            splashSize = QtCore.QSize(
                int(geometry.width() / 4), int(geometry.height() / 4))
        splashImage = QtGui.QPixmap(filename).scaled(
            splashSize, QtCore.Qt.AspectRatioMode.KeepAspectRatio)
        #self.splash = QtWidgets.QSplashScreen(splashImage, QtCore.Qt.WindowStaysOnTopHint)
        QtWidgets.QSplashScreen.__init__(
            self, splashImage, QtCore.Qt.WindowType.WindowStaysOnTopHint)
